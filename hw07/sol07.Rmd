...
```{r run_time}
end_time <- Sys.time()
bake(file=paste0(results_dir,"run_time.rds"),end_time-start_time) -> run_time
```

* On a MacBook Air laptop, the SEIR computations took
`r signif(as.numeric(readRDS("results_2/run_time.rds"))/60,2)`
min at run level 2, and 
`r signif(as.numeric(readRDS("results_3/run_time.rds"))/60,2)`
min at run level 3, with `r readRDS("results_2/cores.rds")` cores.

* On greatlakes, the computation time was
`r signif(as.numeric(readRDS("greatlakes_2/run_time.rds"))/60,2)`
min at run level 2, and
`r signif(as.numeric(readRDS("greatlakes_3/run_time.rds"))/60,2)`
