...
\item We look at monthly time series data on the level of Lake Michigan, which is the same as Lake Huron.
\ei
...
Here is the head of the file \url{gt_lakes_level.csv}

\begin{verbatim}
# Great Lakes level in meters. Downloaded on Jan 19, 2022 from
# https://www.lre.usace.army.mil/Missions/Great-Lakes-Information/Great-Lakes-Information-2/Water-Level-Data/GLHYD_data_metric.csv
...
<<read_data>>=
dat <- read.table(file="gt_lakes_level.csv",sep=",",header=TRUE)
dat$Date <- 1918 + rep(0:11,nrow(dat)/12)/12
head(dat,2)
@
...
dat <- subset(dat,month=="jan")
michigan_level <- dat$Michigan.Huron
year <- dat$year
plot(michigan_level~year,type="l")
@
...
michigan_aic_table <- aic_table(michigan_level,4,5)
require(knitr)
kable(michigan_aic_table,digits=2)
@
...
michigan_arma21 <- arima(michigan_level,order=c(2,0,1))
michigan_arma21
@
...
<<michigan_roots>>=
AR_roots <- polyroot(c(1,-coef(michigan_arma21)[c("ar1","ar2")]))
AR_roots
@
...
\item However, the MA root is $\Sexpr{round(-1/coef(michigan_arma21)["ma1"],2)}$, showing that the fitted model is at the threshold of non-invertibility.
...
<<michigan_profile_code,echo=T,eval=F>>=
K <- 500
ma1 <- seq(from=0.2,to=1.1,length=K)
profile_loglik <- rep(NA,K)
for(k in 1:K){
   profile_loglik[k] <- logLik(arima(michigan_level,order=c(2,0,1),
...
<<michigan_profile,echo=F,fig.width=5,fig.height=2.5,out.width="9cm">>=
par(mai=c(0.8,0.8,0.1,0.1))
<<michigan_profile_code>>
@
...
<<simB>>=
J <- 1000
michigan_ar1 <- arima(michigan_level,order=c(1,0,0))
params <- coef(michigan_ar1)
ar <- params[grep("^ar",names(params))]
intercept <- params["intercept"]
sigma <- sqrt(michigan_ar1$sigma2)
t1 <- system.time(
  michigan_sim <- foreach(j=1:J) %dopar% {
    Y_j <- arima.sim(list(ar=ar),n=length(michigan_level),
      sd=sigma)+intercept
...
<<out, cache=FALSE>>=
sum(sapply(michigan_sim, function(x) inherits(x,"try-error"))) 
@
...
ma1 <- unlist(lapply(michigan_sim,function(x)
...
\item Thus, we cannot reject an AR1 hypothesis for the Michigan data, even though the Fisher information based analysis appears to give strong evidence that the data should be modeled with a nonzero MA1 coefficient. 
...
require(knitr)
kable(michigan_aic_table,digits=1)
@
\end{center}
