...
\begin{itemize}
\item After running \code{module load R} to make R avaialable, you can start an R session in a terminal on the login node just by typing \code{R}.
...
[ionides@gl-login1 ~]$ R
> \hlkwd{install.packages}(\hlstr{"tidyverse"})
> \hlkwd{install.packages}(\hlstr{"pomp"})
\end{alltt}
\end{kframe}
\end{knitrout}
