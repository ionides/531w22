...
This is a two week homework and will count for twice the credit of other one week homeworks. One goal of the homework is to make sure that everyone knows how to write a report as an Rmd file, which is required for the midterm and final projects. Consequently you should submit two files, an Rmd file and the HTML document you obtain by compiling it. The Rmd file should be written so that the grader can run it, so the data should be read from the homework website rather than from a local directory. The grader will not necessarily recompile the Rmd, but if they do then it should reproduce the HTML. If technical difficulties arise with learning to use Rmd format, please consult your peers, piazza, the GSI or myself. 
